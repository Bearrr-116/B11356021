...
flowchart TD
    A[管理員進入登入畫面] --> B[輸入 account 與 password]
    B --> C[系統驗證帳密]
    C --> D{驗證成功?}
    D -- 是 --> E[login_status=success]
    D -- 否 --> F[login_status=failed]
    E --> G[進入管理後台]
    F --> H[提示重新登入]
```
...
%%{init: {"theme":"base","themeVariables":{"primaryColor":"#ffffff","secondaryColor":"#ffffff","tertiaryColor":"#ffffff","mainBkg":"#ffffff","lineColor":"#333333","textColor":"#111111"}}}%%
flowchart TD
    A[進入買家帳號管理] --> B[輸入 user_id 或條件查詢]
    B --> C[顯示買家帳號資料]
    C --> D{是否調整 account_status?}
    D -- 是 --> E[更新 account_status]
    D -- 否 --> F[維持現狀]
    E --> G[紀錄 updated_at]
    F --> G
```
...
%%{init: {"theme":"base","themeVariables":{"primaryColor":"#ffffff","secondaryColor":"#ffffff","tertiaryColor":"#ffffff","mainBkg":"#ffffff","lineColor":"#333333","textColor":"#111111"}}}%%
flowchart TD
    A[進入賣家帳號管理] --> B[查詢 seller user_id]
    B --> C[查看賣家狀態與資料]
    C --> D{是否異動帳號?}
    D -- 是 --> E[更新 account_status / suspend_status]
    D -- 否 --> F[結束]
    E --> G[寫入 updated_at]
    G --> F
```
...
flowchart TD
    ...
    B --> C{達停權標準?}
    C -- 是 --> D[填寫 suspend_reason]
    D --> E[設定 suspend_date]
    E --> F[更新 suspend_status]
    C -- 否 --> G[不執行停權]
```
...
%%{init: {"theme":"base","themeVariables":{"primaryColor":"#ffffff","secondaryColor":"#ffffff","tertiaryColor":"#ffffff","mainBkg":"#ffffff","lineColor":"#333333","textColor":"#111111"}}}%%
flowchart TD
    A[管理員進入身份審核] --> B[查詢 user_id]
    B --> C[檢視 identity_no 與文件]
    C --> D{審核是否通過?}
    D -- 通過 --> E[verification_status=approved]
    D -- 不通過 --> F[verification_status=rejected]
    E --> G[寫入 verification_date]
    F --> G
```
...
%%{init: {"theme":"base","themeVariables":{"primaryColor":"#ffffff","secondaryColor":"#ffffff","tertiaryColor":"#ffffff","mainBkg":"#ffffff","lineColor":"#333333","textColor":"#111111"}}}%%
flowchart TD
    A[管理員查看待審商品] --> B[開啟 product_id 詳情]
    B --> C{符合規範?}
    C -- 是 --> D[audit_status=approved]
    C -- 否 --> E[audit_status=rejected]
    D --> F[寫入 audit_date]
    E --> F
```
...
%%{init: {"theme":"base","themeVariables":{"primaryColor":"#ffffff","secondaryColor":"#ffffff","tertiaryColor":"#ffffff","mainBkg":"#ffffff","lineColor":"#333333","textColor":"#111111"}}}%%
flowchart TD
    A[接收違規商品通報] --> B[查核 product_id]
    B --> C{是否違規?}
    C -- 是 --> D[商品下架]
    D --> E[更新 audit_status=takedown]
    E --> F[通知賣家]
    C -- 否 --> G[保留上架狀態]
```
...
%%{init: {"theme":"base","themeVariables":{"primaryColor":"#ffffff","secondaryColor":"#ffffff","tertiaryColor":"#ffffff","mainBkg":"#ffffff","lineColor":"#333333","textColor":"#111111"}}}%%
flowchart TD
    A[管理員進入手續費設定] --> B[輸入 fee_rate 與生效日]
    B --> C[系統驗證費率範圍]
    C --> D{驗證通過?}
    D -- 是 --> E[儲存 fee_rate]
    E --> F[寫入 effective_date]
    D -- 否 --> G[提示修正]
```
